...
class DeviceEdit extends React.Component {
  ...
  componentWillMount() {
    const { device, location, deviceManagement } = this.props;
    let params = searchFormat(location.search);
    let cameraInfo = toJS(device.queryCameraById(params.id));
    if (!cameraInfo) {
      this.setState({ noData: true, initData: true });
      return;
    }

    Service.device.queryDeviceVo(params.id)
    .then(res => {
  ...
  updateCameraInfo() {
    ...
    let options = {
      ...
      installationSite:deviceManagement.formData.installationSite,
