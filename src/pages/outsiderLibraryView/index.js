...
// 加载基础组价
const LibList = Loader.loadModuleComponent('MonitorLibrary', 'LibList')
...
@observer
class outsiderLibraryView extends React.Component {
  constructor(props) {
    super(props)
    this.libTypeInfo = libTypeMap;
    this.libType = libTypeMap.libType;
    this.libLabel = libTypeMap.libLabel;
  ...
  componentWillMount(){
    ...
    monitorLib.initData({ libType: this.libTypeInfo.libType })
  }
  ...
  setDeleteLibPeopleContent = (peopleIds, libId, callback) => {
    ...
    return Promise.resolve(() => this.setState({ loading: false }))
  }
  ...
  deletePeopleBatch = () => {
    ...
    return Service.monitorLib.deleteMonitorLibPersons(peopleIds, libDetail).then(res => {
      if(res.code === 200000){
        message.success("删除成功");
  ...
  getAsyncLibList = (libId) => {
    ...
    const { monitorLib } = this.props;
    return Service.monitorLib.queryMonitorLibs(monitorLib.searchData).then(async result => {
      if(!result.data){
        return false
      }
  ...
  getAsyncLibDetail(id){
    return Service.monitorLib.queryMonitorLibDetail(id).then(result => {
      ...
      if(Array.isArray(libDetail.objectMainList)){
  ...
  */
  ...
  render() {
    const { content, libDetail, libList, currLibId, modalVisible, spinning, spinningTip } = this.state;
    let modalOptions;
    if(modalVisible) {
          ...
          ? <div className='monitee-black-lib-detail-wrapper'>
              <React.Fragment>
                ...
                <LibPeople
                  ...
                  actionName='outsiderLibraryManage'
                />
              </React.Fragment>
            </div>
...
export default outsiderLibraryView